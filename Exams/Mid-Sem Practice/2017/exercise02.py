...
def f(arg_for_seed, nb_of_elements, max_size):
    ...
    for i in L:
        ...
        if len(x) is 1:
            numbers_made_up_of_increasing_digits_per_length[len(x)].append(int(x))
        else:
    ...
    for key in sorted(numbers_made_up_of_increasing_digits_per_length):
        print(f'List members of length {key} with no successive occurrences of the same digit:')
        print('\t', end='')
        
        ctr = 0
        for val in numbers_made_up_of_increasing_digits_per_length[key]:
            ctr += 1
            
            if ctr is len(numbers_made_up_of_increasing_digits_per_length[key]):
                print(val, end='')
            else:
                print(val, end=' ')
