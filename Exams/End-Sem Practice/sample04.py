...
def is_word_word_ladder(word_1, word_2, candidate_ladder):
    ...
    if candidate_ladder[0] != word_1 or candidate_ladder[-1] != word_2 or word_1 not in lexicon:
        return False
    
    for i in range(1, len(candidate_ladder)):
    ...
    possible_ladders = deque([[word_1]])
    
    while possible_ladders:
        x = possible_ladders.popleft()
        
        for i in closest_words[x[-1]]:
            temp = list(x)
            temp.append(i)
            
            if i == word_2 and temp == candidate_ladder:
                return True
            
            if i == word_2 and len(temp) < len(candidate_ladder):
                return False
            
            if i not in x:
                possible_ladders.append(temp)
